...
import { Upload, Trash2, Download, Image as ImageIcon, File, Search, Video, Music, FileText, Package, X, Edit } from 'lucide-react';
  ...
  const filteredAssets = useMemo(() => {
    return assets.filter((asset) => {
      ...
      if (searchQuery.trim()) {
        ...
        return filename.includes(query) || asset.key.toLowerCase().includes(query);
      }
              ...
              {filteredAssets.map((asset) => {
                ...
                return (
                  <tr key={asset.key} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-6 py-4 font-medium max-w-xs truncate" title={asset.key}>
                      {asset.key?.split('/').pop() || asset.key}
                    </td>
                    <td className="px-6 py-4">
