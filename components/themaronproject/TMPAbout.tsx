  ...
  return (
    ...
    >
      <div className="pointer-events-none absolute inset-0 -z-10">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-b from-gray-900/80 via-gray-900/75 to-black/85" />
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,rgba(59,130,246,0.2),transparent_25%),radial-gradient(circle_at_80%_0%,rgba(45,212,191,0.25),transparent_20%),radial-gradient(circle_at_50%_80%,rgba(251,191,36,0.18),transparent_25%)]" />
      ...
      >
        <motion.div variants={titleVariants} className="max-w-3xl space-y-4">
          ...
          <h1 className="text-balance text-3xl text-white sm:text-4xl lg:text-5xl" style={{ fontFamily: 'var(--font-display)', lineHeight: '1.25' }}>
            What is{' '}
            <em><span className="bg-gradient-to-r from-blue-400 via-teal-400 to-amber-300 bg-clip-text font-semibold text-transparent">The Maron Project</span></em>?
          </h1>
          <p className="max-w-2xl text-base leading-relaxed text-gray-100/90">
        ...
        <motion.div
          ...
          className="relative overflow-hidden rounded-3xl border border-white/10 bg-white/10 p-6 shadow-xl shadow-teal-500/5 backdrop-blur"
          role="article"
        >
          <div className="pointer-events-none absolute inset-0 bg-gradient-to-br from-blue-500/8 via-teal-500/8 to-amber-400/10" />
          <div className="relative space-y-6">
            {tmpContent && tmpContent.split('\n').map((paragraph: string, index: number) => (
              <p key={index} className="text-base leading-relaxed text-gray-100/90">
                {paragraph}
              </p>
            ))}
          </div>
        ...
        >
          <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-white/10 p-6 shadow-xl shadow-teal-500/5 backdrop-blur">
            <div className="pointer-events-none absolute inset-0 bg-gradient-to-br from-blue-500/8 via-teal-500/8 to-amber-400/10" />
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {highlights.map((highlight) => (
              <div
                key={highlight.title}
                className="relative overflow-hidden rounded-2xl border border-white/10 bg-white/10 p-5 shadow-md shadow-teal-500/5 backdrop-blur"
              >
                <div className="pointer-events-none absolute inset-0 bg-gradient-to-br from-blue-500/8 via-teal-500/6 to-amber-400/8" />
                <div className="relative space-y-2">
                  <p className="text-sm font-semibold text-white">{highlight.title}</p>
                  <p className="text-sm text-gray-100/90">{highlight.description}</p>
                </div>
              </div>
            ))}
          </div>
